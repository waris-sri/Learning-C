...
int search(char *prefix, char *out, struct Student students[], int size) {
  char temp[MAX_LENGTH]; // For holding matching strings
  int found = 0;         // Assume as not found first
  for (int i = 0; i < size; i++) {
    ...
    if (strncasecmp(students[i].name, prefix, strlen(prefix)) == 0) {
      ...
      temp[k] = '\0'; // Terminate the `temp` string
...
int main() {
  ...
  scanf("%s", line);
  int is_match = search(line, suggestion, students, SIZE);
  if (!is_match) {
